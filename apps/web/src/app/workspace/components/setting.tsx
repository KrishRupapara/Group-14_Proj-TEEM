...
export default function Setting() {
  ...
  const onFormSubmit = (data) => {
    ...
    let description = workspace.description;
    try {
      const res = fetch('http://localhost:3500/api/' + id + '/editWSDetails', {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  useEffect(() => {
    ...
    const once = () => {
      ...
      axios.get('http://localhost:3500/api/' + id + '/editWSDetails')
        .then(res => {
          setWorkspace({ ...workspace, title: res.data.title, type: res.data.type, description: res.data.description, members: res.data.Members })
        }/*console.log(res)*/)
        ...
        <div className='flex flex-col justify-around'>
          ...
          <form action="" onSubmit={handleSubmit(onFormSubmit)}>
            <div className='flex flex-col'>
              ...
              <label htmlFor="member" id="member" className="font-bold mb-1 mt-2">
                member
              </label>
              ...
              <div className='flex flex-col justify-items-start' >
                {fields.map(({ id }, index) => {
                  return <div className='lg:flex'>
                    ...
                    <select id="Role" required className='border rounded-xl p-2 m-1' value={members[index].Role} {...register(`members.${index}.Role` as any)}>
              ...
              {/* </div> */}
              <button type="submit" className='border bg-blue-600 p-3 mt-3 rounded-xl py-1 mx-auto w-fit'>Continue</button>
