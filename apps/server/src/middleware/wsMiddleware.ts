...
export const authorizeManager = async(req: Request, res: Response, next: NextFunction) =>{
...
export const authorizeMember = async (req: Request, res: Response, next: NextFunction) => {
...
const isManager = await db
 ...
 if(isManager.length == 0)
   res.send("Workspace doesn't exist");
...
if(isManager[0].projectManager !== userID)
{
  const isMemeber = await db
   .select()
   .from(members)
   .where(and(eq(members.workspaceID, workspaceID.wsID), eq(members.memberID, userID)))
   .limit(1)
